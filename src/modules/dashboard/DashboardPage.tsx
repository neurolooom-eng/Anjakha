...
import {
  Activity, AlertTriangle, BedDouble, IndianRupee, Receipt, ShieldCheck, Users,
} from 'lucide-react'
import {
...
export function DashboardPage() {
  ...
  const moduleCards = NAV_GROUPS.flatMap((g) => g.items).filter(
    (item) => item.path !== '/' && item.permission && hasPermission(item.permission),
  )
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-muted">Live snapshot across clinical, pharmacy, and finance operations.</p>
      </div>
